	...
	const FMassBehaviorEntityContext& EntityContext) const
{
	...
	if (EntityContext.SmartObjectSubsystem.GetWorld() && UGameplayStatics::GetPlayerPawn(EntityContext.SmartObjectSubsystem.GetWorld(), 0))
	{
		...
		const FMassEntityTemplate EntityTemplate = ItemConfig->GetConfig().GetOrCreateEntityTemplate(*UGameplayStatics::GetPlayerPawn(EntityContext.SmartObjectSubsystem.GetWorld(), 0)->GetWorld(), *ItemConfig);
		...
		{
			CommandBuffer.PushCommand<FMassDeferredCommand>([SOComp, EntityContext](UMassEntitySubsystem& System)
			{
